...
async function openEditor(filename = null) {
    ...
    currentCollection = pathParts[pathParts.length - 2] || pathParts[pathParts.length - 1];
    if (currentCollection === 'index.php' || currentCollection === '') {
...
async function savePoem() {
    ...
    try {
        ...
        if (result.success) {
            window.location.href = `?view=poem&poem=${encodeURIComponent(result.filename)}`;
        } else {
