...
// Cookie Fallback Secret
define('COOKIE_SECRET', md5($adminPasswordHash));
...
if (file_exists($configPath) && is_readable($configPath)) {
    ...
    if ($content !== false) {
        ...
        if (is_array($decoded) && !empty($decoded['admin_password_hash'])) {
            $adminPasswordHash = $decoded['admin_password_hash'];
        }
    }
}
...
if (isset($_POST['admin_login'])) {
    ...
    if (password_verify($password, $adminPasswordHash)) {
        set_admin_auth(true);
        header('Location: ' . getCurrentBaseUrl() . ($_SERVER['QUERY_STRING'] ? '?' . preg_replace('/&?admin=1&?/', '', $_SERVER['QUERY_STRING']) : ''));
        exit;
    } else {
        ...
        $params = $_GET;
        $params['admin'] = '1';
        header('Location: ' . getCurrentBaseUrl() . '?' . http_build_query($params));
        exit;
    }
}
        ...
        * {
        ...
        @media (max-width: 600px) {
            .breadcrumbs {
                ...
                /* Extra right padding to clear theme toggle */
                overflow-x: auto;
...
<body class="view-<?= $view ?><?= $isAdmin ? ' admin-mode' : '' ?>">
    ...
    <div class="container">
        <div class="breadcrumbs">
            ...
            <?php else: ?>
                ...
                <span class="breadcrumb-separator">›</span>
                <span
                    class="breadcrumb-current"><?= $breadcrumbPoemTitle ?: str_replace(['_', '-'], [' ', ' '], preg_replace('/_(en|el)$/', '', pathinfo($file, PATHINFO_FILENAME))) ?></span>
            <?php endif; ?>
        </div>
                    ...
                    <div class="dropdown-content">
                        ...
                        <form action="../../index.php" method="POST" enctype="multipart/form-data">
                            ...
                            <input type="text" name="author_name"
                                value="<?= htmlspecialchars($config['author_name'] ?? '') ?>" required>
    ...
    <?php if ($showAdminLogin): ?>
        <div class="login-overlay"
            onclick="if(event.target === this) window.location='<?= getCurrentBaseUrl() . ($_SERVER['QUERY_STRING'] ? '?' . preg_replace('/&?admin=1&?/', '', $_SERVER['QUERY_STRING']) : '') ?>'">
            <div class="login-box">
